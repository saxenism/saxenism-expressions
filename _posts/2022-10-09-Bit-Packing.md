...
Next is the biggest pitfall of all. This is where I tripped hard and my mind almost broke (since I could not make sense of the ERC721A contract because of this assumption of mine). Since, most of might have native languages where we read that language from left to right you would naturally expect Solidity to also be storing data the same way (from left to right). 

*BUT BUT BUT*

That is not the case. Solidity stores data starting from left, meaning it is (probably) Little Endian. A good primer about Endianess can be found in [this freecodecamp article](https://www.freecodecamp.org/news/what-is-endianness-big-endian-vs-little-endian/).
